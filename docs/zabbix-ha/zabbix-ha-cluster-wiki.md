# Zabbix HA-кластер «Terem Zabbix» — полное описание (Wiki)
...
## 12. Открытые хвосты (не блокеры)

1. **Watchdog на одном прокси (Литва).** Одновременный отказ прокси и кластера = тревога не уйдёт. Добавить второй путь (NL/xray `72.56.72.132`) второй строкой в `PROXIES`.
2. **Токен Telegram** засвечен — перевыпустить у @BotFather при необходимости.
3. **zabbix_agent2.conf на node2** сбрасывался на дефолт при апгрейде — проверить кастомные настройки.
4. **PostgreSQL ssl=off**, фронтенд `ENCRYPTION=true` — работает; при желании включить ssl в Patroni.
5. **Failover delay 60с** — можно уменьшить до 15–30с.
6. **Hostname агентов** = «Zabbix server» на обоих; для активных проверок должен совпадать с именем хоста в Zabbix (пассивные работают).

---

*Документ отражает состояние кластера на 2026-07-16. Хранить в wiki, обновлять при изменениях.*
